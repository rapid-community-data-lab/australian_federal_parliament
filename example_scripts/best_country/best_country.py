"""
This script grabs all existing speeches that contain the phrase 'best country'.
...
query = """
    WITH matching_speech as (
        ...
        where lower(paragraph_text) glob ?1
    )
    select 
        session.date,
        session.chamber,
        ...
        lower(paragraph_text) glob ?1 as matches_phrase
    from paragraph
    inner join session using(session_id)
...
results = db.execute(query, [pattern])
...
workbook.save("best_country.xlsx")
